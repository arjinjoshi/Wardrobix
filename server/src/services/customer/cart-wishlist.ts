...
export const addToCart = async (
  ...
  return await getCartResponse(userId);
};
...
export const syncCart = async (userId: string, incomingItems: SyncCartItemInput[]) => {
  let cart = await Cart.findOne({ user: userId });
...
export const addToWishlist = async (userId: string, productId: string) => {
  ...
  return await getWishlistResponse(userId);
};

export const deleteFromWishlist = async (userId: string, productId: string) => {
  let wishlist = await Wishlist.findOne({ user: userId });
